      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {coursesData.map((course) => {
            return <CourseCard key={course.id} course={course} isAuthenticated={isAuthenticated} />;
          })}
        </div>
...
const CourseCard = ({ course, isAuthenticated }: { course: any, isAuthenticated: boolean }) => {
  const { progress } = useCourseProgress(course.id);
  const progressPercentage = progress?.progress_percentage || 0;
  const totalLessons = course.lessons.length;

  return (
    <Card className="hover:shadow-lg transition-shadow duration-300 border-cactus-200">
      <CardHeader>
        ...
        {isAuthenticated && progressPercentage > 0 && (
          <div className="mb-4">
            ...
            <p className="text-xs text-cactus-500 mt-1">
              {Math.round((progressPercentage / 100) * totalLessons)} of {totalLessons} lessons completed
            </p>
          </div>
        )}
